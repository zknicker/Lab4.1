...
void main(){
    vec4 p = model * vec4(vertex_model, 1);
    //p.y = 0.5;
    //float top = (light_world.y * vertex_model.x) - (light_world.x * vertex_model.y);
    //float bottom = (light_world.y - vertex_model.y);
    //p.x = top / bottom;
